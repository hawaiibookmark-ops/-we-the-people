#!/usr/bin/env python3
"""Official Oregon federal votes from House Clerk EVS and Senate LIS."""
...
def load_house_delegation() -> dict[str, tuple[str, str]]:
    ...
    for mem in root.findall(".//member"):
        ...
        if statedist.startswith("OR") and statedist[2:].isdigit():
            dist = f"OR-{int(statedist[2:]):02d}"
        else:
            raise SystemExit(f"unexpected Clerk statedistrict {statedist!r}")
        if not bio or not name:
            raise SystemExit("Clerk MemberData missing official OR name/bioguide")
        out[bio] = (name, dist)
    if len(out) != 6:
        raise SystemExit(f"expected 6 sitting OR House members from Clerk, got {len(out)}")
    return out
...
def load_senate_delegation() -> dict[tuple[str, str], tuple[str, str]]:
    ...
    for mem in root.findall(".//member"):
        ...
        if not last or not name or not bio:
            raise SystemExit("Senate contact XML missing official OR senator fields")
        out[(last, st)] = (name, bio)
    if len(out) != 2:
        raise SystemExit(f"expected 2 sitting OR senators from senate.gov, got {len(out)}")
    return out
...
def write_votes(rows: list[dict]) -> None:
    ...
    payload = {
        "policy": (
            ...
            "roll-call XML (119th Congress 2nd session votes 202-231) for sitting OR members. "
...
def write_delegation(house: dict[str, tuple[str, str]], senate: dict[tuple[str, str], tuple[str, str]]) -> None:
    ...
    dest = OUT / "or" / "congress-delegation.json"
    dest.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def write_leg_index() -> None:
    payload = {
        ...
        "sources": [
            {
                "label": "Oregon Legislative Information System (OLIS)",
                "url": "https://olis.oregonlegislature.gov/",
            },
            {
                "label": "Oregon State Legislature (official home)",
                "url": "https://www.oregonlegislature.gov/",
            },
        ],
    }
    dest = OUT / "or" / "legislature-vote-index.json"
    dest.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def merge_stub() -> None:
    stub_path = OUT / "or.json"
    ...
    stub["congress_delegation_path"] = "/data/or/congress-delegation.json"
    stub["legislature_vote_index_path"] = "/data/or/legislature-vote-index.json"
    if donors:
...
def main() -> int:
    ...
    if len(rows) != 300:
        raise SystemExit(f"expected 300 OR federal votes (6x40 + 2x30), got {len(rows)}")
    write_votes(rows)
