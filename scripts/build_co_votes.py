#!/usr/bin/env python3
"""Official Colorado federal votes from House Clerk EVS and Senate LIS."""
...
RETRIEVED = "2026-09-02T11:24:04Z"
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "public" / "data"
CACHE = Path("/tmp/votes-co")
...
def load_house_delegation() -> dict[str, tuple[str, str]]:
    ...
    for mem in root.findall(".//member"):
        ...
        if statedist.startswith("CO") and statedist[2:].isdigit():
            dist = f"CO-{int(statedist[2:]):02d}"
        else:
            raise SystemExit(f"unexpected Clerk statedistrict {statedist!r}")
        if not bio or not name:
            raise SystemExit("Clerk MemberData missing official CO name/bioguide")
        out[bio] = (name, dist)
    if len(out) != 8:
        raise SystemExit(f"expected 8 sitting CO House members from Clerk, got {len(out)}")
    return out
...
def load_senate_delegation() -> dict[tuple[str, str], tuple[str, str]]:
    ...
    for mem in root.findall(".//member"):
        ...
        if not last or not name or not bio:
            raise SystemExit("Senate contact XML missing official CO senator fields")
        out[(last, st)] = (name, bio)
    if len(out) != 2:
        raise SystemExit(f"expected 2 sitting CO senators from senate.gov, got {len(out)}")
    return out
...
def write_votes(rows: list[dict]) -> None:
    ...
    payload = {
        "policy": (
            ...
            "roll-call XML (119th Congress 2nd session votes 202-231) for CO members. "
    ...
    dest = OUT / "co" / "votes.json"
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"wrote {dest} count={len(rows)} members={len(counts)}", flush=True)


def write_delegation(house: dict[str, tuple[str, str]], senate: dict[tuple[str, str], tuple[str, str]]) -> None:
    payload = {
        "state": "CO",
        "retrieved_at": RETRIEVED,
    ...
    dest = OUT / "co" / "congress-delegation.json"
    dest.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def write_leg_index() -> None:
    payload = {
        ...
        "sources": [
            {
                "label": "Colorado General Assembly bill information",
                "url": "https://leg.colorado.gov/bills",
            },
            {
                "label": "Colorado General Assembly (official legislature home)",
                "url": "https://leg.colorado.gov/",
            },
        ],
    }
    dest = OUT / "co" / "legislature-vote-index.json"
    dest.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def merge_stub() -> None:
    stub_path = OUT / "co.json"
    ...
    stub["votes_path"] = "/data/co/votes.json"
    stub["congress_delegation_path"] = "/data/co/congress-delegation.json"
    stub["legislature_vote_index_path"] = "/data/co/legislature-vote-index.json"
    if donors:
...
def main() -> int:
    ...
    if len(rows) != 380:
        raise SystemExit(f"expected 380 CO federal votes (8x40 + 2x30), got {len(rows)}")
    write_votes(rows)
    write_delegation(house, senate)
    write_leg_index()
    merge_stub()
    return 0
