#!/usr/bin/env python3
"""Wave-3 federal FEC Schedule A $200+ extracts (PA/OH/GA/NC/NJ) from official indiv26 + cn26 + ccl26."""
...
# Origin wave-3 published kept_rows (official indiv26 scan).
EXPECT_KEPT = {
    "PA": 30261,
    "OH": 37133,
    "GA": 52119,
...
def scope_text(state: str) -> str:
    name = NAMES[state]
    return (
        f"{name} 2026 House/Senate federal FEC Schedule A $200+ only. "
        "State campaign-finance bulk is pending (no standing statewide Schedule A extract in this populate)."
    )
...
def build_payload(state: str, cands: list[dict], owners: dict[str, str], by_cmte: dict[str, list[dict]]) -> dict:
    ...
    expected = EXPECT_KEPT[state]
    if kept_unique != expected:
        print(f"NOTE {state} official kept_rows={kept_unique} (Origin cited {expected})", flush=True)
    policy = (
        ...
        "This is federal FEC only; statewide campaign-finance bulk is pending. "
...
def write_stub(state: str, payload: dict) -> None:
    stub_path = OUT / f"{state.lower()}.json"
    if stub_path.exists():
        stub = json.loads(stub_path.read_text(encoding="utf-8"))
        # Never wipe existing sourced donors, ballots, or votes.
        existing_donors = ((stub.get("state_filings") or {}).get("donors") or {})
        if existing_donors.get("status") == "sourced" or (
            existing_donors.get("path") and "fec-donors" not in str(existing_donors.get("path") or "")
        ):
            raise SystemExit(f"refusing to wipe sourced donors on {stub_path}")
    else:
        stub = {
            "election": {
                "jurisdiction": NAMES[state],
                "state_code": state,
                "general_date": "2026-11-03",
                "note": (
                    f"Official {NAMES[state]} 2026 House/Senate federal FEC Schedule A $200+. "
    ...
    filings["state_donors"] = {
        "status": "pending",
        "reason": (
            f"{NAMES[state]} statewide campaign-finance bulk is not extracted in this populate. "
            "State donor names are not invented. Federal FEC Schedule A $200+ is partial."
        ),
        "do_not_sell_donor_lists": True,
    }
    ...
    print(f"wrote stub {stub_path} fec candidates={payload['candidate_count']} kept={payload['row_count']}", flush=True)
...
def main() -> int:
    ...
    print(f"wave3 unique_pcc={len(pccs)} ccl_p_owners={len(owners)}", flush=True)
